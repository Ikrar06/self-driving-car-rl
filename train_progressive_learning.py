"""
Progressive Learning Training - Agent learns to fix mistakes from previous episode.
Uses LOW epsilon so agent is consistent and improves gradually.
"""
...
def train_progressive_learning(
    track_path: str = "tracks/f1_grand_circuit.json",
    num_episodes: int = 200,
    fps: int = 20,
):
    """
    Train agent with PROGRESSIVE LEARNING - learns to fix mistakes each episode.
    ...
    """
    print("=" * 70)
    print("📚 PROGRESSIVE LEARNING MODE")
    print("=" * 70)
    print("Agent will learn to FIX MISTAKES from previous episode!")
    print("LOW EPSILON = More consistent, less random")
    print("=" * 70)

    # Load config
    modes_config = load_config("config/training_modes.yaml")
    mode = modes_config['dqn_progressive']  # Progressive mode config
    ...
    print(f"✓ Car size: {env_config['car']['width']}x{env_config['car']['height']}")
    print(f"✓ FPS: {fps}")
    ...
    # Create agent with LOW epsilon for consistency
    print("\n🤖 Creating DQN agent with LOW EPSILON...")
    agent = DQNAgent(
    ...
    print(f"✓ Starting Epsilon: {agent.epsilon:.3f} (LOW for consistency!)")
    ...
    print("\n💡 LEARNING STRATEGY:")
    print("   - Agent exploits what it learned (low randomness)")
    print("   - Each episode, agent tries to do better than last")
    print("   - RED GHOSTS = Previous episode's mistakes")
    print("   - BRIGHT CAR = Current attempt to fix mistakes")
    print("=" * 70 + "\n")
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            ...
            # Select action (mostly exploit due to low epsilon)
            ...
            info_texts = [
                ...
                f"Previous Length: {len(ghost_renderer.previous_episode_path)}",
                f"Best Length: {best_length}",
                f"Reward: {episode_reward:.1f}",
                f"Epsilon: {agent.epsilon:.3f} (LOW)",
        ...
        progress = f"+{episode_length - len(ghost_renderer.previous_episode_path)}" if len(ghost_renderer.previous_episode_path) > 0 else ""

        print(f"Ep {episode+1:3d}: Length={episode_length:3d} {progress}, "
        ...
        if episode_reward > best_reward:
            ...
            save_path = checkpoint_dir / "best_model_progressive.pt"
            agent.save(str(save_path))

        # Periodic checkpoint
        if (episode + 1) % 50 == 0:
            save_path = checkpoint_dir / f"progressive_ep{episode + 1}.pt"
            agent.save(str(save_path))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--track", type=str, default="tracks/f1_grand_circuit.json")
    parser.add_argument("--episodes", type=int, default=200)
    parser.add_argument("--fps", type=int, default=20)
    args = parser.parse_args()

    train_progressive_learning(
